# Correspondencia asociativa con sustitución secuencial
...
class Cache:
  ...
  def agregar_linea(self, desde, hasta, pos):
    ...
    # buscar secuencialmente si hay una linea vacia partiendo desde pos
    buscar_pos = pos + 1

    while buscar_pos % self.size != pos:
      if self.lineas[buscar_pos % self.size].disponible:
        # encontramos una linea disponible
        self.lineas[buscar_pos % self.size] = Linea(False, desde=desde, hasta=hasta)
        return
      buscar_pos = buscar_pos + 1

    # no encontramos una linea disponible, lo insertamos en pos
    self.lineas[pos] = Linea(False, desde=desde, hasta=hasta)
